...
・这里也是只有战斗胜利才会进行通信，玩家对战获胜后能得到金币，宝箱以及奖杯，获得的金币和奖杯根据对战玩家和自己的奖杯差来决定，计算公式看后面的`奖杯获取公式`和`金币获取公式`，这里还需要一个特殊处理，因为输的人不需要通信，所以赢的人通信的时候要把对手的奖杯数也一起扣掉，就是说自己的奖杯数增加，对手的奖杯数减少。 
#### 返回值
    {
        playerModel:playerModel,
        boxId:1,
